...
TruckingCompany[0] = {["x"] = 1195.90,["y"] = -3254.28, ["z"] = 7.09518}
local TruckingTrailer = {}
TruckingTrailer[0] = {["x"] = 1199.83, ["y"] = -3237.42, ["z"] = 6.02876}
...
local MissionData = {
    ...
    [4] = {196.5617, 6631.0967, 31.53, 25000}
}
local MISSION = {}
MISSION.start = false
...
function init()
    ...
    SetBlipAsMissionCreatorBlip(BLIP.company, true)
...
Citizen.CreateThread(function()
	while true do
        Wait(0)
		DrawMarker(2,-7.10,-0.30,73.07,0.0, 0.0, 0.0, 180.0, 0.0, 0.0, 0.75, 0.75, 0.75, 255, 0, 0, 100, false, true, 2, false, false, false, false)
		drawTxt('Appuyez sur ~g~E~s~ pour commencer a travailler.',0,1,0.5,0.8,0.6,255,255,255,255)
	end
end)
...
function tick()
    ...
    elseif( MISSION.start == true ) then
        ...
        if( IsEntityAttached(MISSION.trailer) and text1 == false) then
            TriggerEvent("mt:missiontext", "Conduire à la destination ~g~marquée~w~.", 10000)
            text1 = true
        ...
        if ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and  not IsEntityAttached(MISSION.trailer)) then
            ...
            TriggerEvent("mt:missiontext", "Tu as gagné ~g~$"..currentMission[4], 5000)
        ...
        elseif ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and IsEntityAttached(MISSION.trailer) ) then
            TriggerEvent("mt:missiontext", "Arrivée. Détachez votre ~o~remorque~w~ avec ~r~H~w~", 15000)
        end
...
function MISSION.spawnTruck()
    ...
    MISSION.truck = CreateVehicle(MISSION.hashTruck, 1192.63, -3204.40, 6.02803, 0.0, true, false)
...
function GUI.init()
    ...
    GUI.addButton(1, "Mission 5 [ 25.000$ ]", GUI.mission, 0.35, 0.45, 0.3, 0.05)
