...
function StartJob(jobid)
    ...
    jobs.coords.cx[92],jobs.coords.cy[92],jobs.coords.cz[92] = -798.090, 831.699, 204.351
		...
		Wait(2000)
		DrawMissionText("Circuler et chercher des ~h~~y~passagers~w~.", 10000)
		onJob = jobid
	end
end
...
Citizen.CreateThread(function()
	while true do
		Wait(0)
		if onJob == 0 then
			if IsControlJustPressed(1, 38) or IsDisabledControlJustPressed(1, 214) then -- DEL
				...
				if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
					if DoesEntityExist(jobs.peds[1]) then
						if IsPedFatallyInjured(jobs.peds[1]) then
							...
							DrawMissionText("Le client est ~r~mort~w~. Trouver un autre client.", 5000)
						else
							if jobs.flag[1] == 1 and jobs.flag[2] > 0 then
								...
								if jobs.flag[2] == 0 then
									...
									Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
									jobs.peds[1] = nil
									DrawMissionText("Le client a été ~r~fatigué d'attendre~r~. Trouver un autre client.", 5000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								else
									if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
							...
							if jobs.flag[1] == 2 and jobs.flag[2] > 0 then
								...
								if jobs.flag[2] == 0 then
									...
									Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
									jobs.peds[1] = nil
									DrawMissionText("~r~Le client ne va pas avec vous~w~. Trouver un autre.", 5000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								else
									if IsPedSittingInVehicle(jobs.peds[1], jobs.cars[1]) then
										...
										if street[2] ~= 0 and street[2] ~= nil then
											local streetname = string.format("~w~Emmenez-moi à~y~ %s~w~, proche de~y~ %s", GetStreetNameFromHashKey(street[1]),GetStreetNameFromHashKey(street[2]))
											DrawMissionText(streetname, 5000)
										else
											local streetname = string.format("~w~Emmenez-moi à~y~ %s", GetStreetNameFromHashKey(street[1]))
											DrawMissionText(streetname, 5000)
										end
                  ...
                  DrawMissionText("~g~Vous avez livré le client!", 5000)
									-- pay money on something
									Wait(8000)
									DrawMissionText("Circuler et chercher des nouveaux ~h~~y~clients~w~.", 10000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 90)
								end
							end
						...
						if jobs.flag[1] > 0 then
							jobs.flag[1] = 0
							jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
							DrawMissionText("Circuler et chercher des ~h~~y~clients~w~.", 10000)
							if jobs.blip[1] ~= nil and DoesBlipExist(jobs.blip[1]) then
								Citizen.InvokeNative(0x86A652570E5F25DD,Citizen.PointerValueIntInitialized(jobs.blip[1]))
								jobs.blip[1] = nil
							end
						...
						if jobs.flag[1] == 0 and jobs.flag[2] > 0 then
							...
							if jobs.flag[2] == 0 then
								...
								if DoesEntityExist(rped) then
									...
									DrawMissionText("Le ~g~client~w~ vous attend. Conduire à proximité", 5000)
					...
					if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), GetEntityCoords(jobs.cars[1]), true) > 30.0001 then
						...
						DrawMissionText("Revenez dans votre voiture pour ~y~continuer~w~. Ou éloignez-vous du taxi pour ~r~arrêter~ la mission.", 1)
					end
				end
			else
				StopJob(1)
				DrawMissionText("Le taxi est ~h~~r~détruit~w~.", 5000)
			end
		end
	end
end)
